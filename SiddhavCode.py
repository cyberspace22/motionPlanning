...
def movecost(curr,coord,action,cost):
    ...
    coord[3] = actn
    return mcost
def compute_plan(grid,start,goal,cost,heuristic,action,dir_name,plan):
    ...
    while True:
        ...
        for coord in neigh:#for every accessible neighbor found
            cst = current[1] + movecost(current,coord,action,cost)
